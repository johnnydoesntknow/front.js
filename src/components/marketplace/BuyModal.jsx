...
const BuyModal = ({ asset, onClose, onPurchase }) => {
  const { showNotification, userKYCStatus, setUserKYCStatus } = useApp();
  ...
  // Check KYC status on mount - ADDED
  useEffect(() => {
    checkKYCStatus();
  }, [address, signer]);

  // KYC check function - ADDED
  const checkKYCStatus = async () => {
    ...
    try {
      ...
      setUserKYCStatus(verified); // Update global state too
    } catch (error) {
  ...
  // UPDATED: Check KYC requirement
  const meetsKYCRequirement = useMemo(() => {
    if (!asset.requiresPurchaserKYC) return true;
    return isKYCVerified === true;
  }, [asset.requiresPurchaserKYC, isKYCVerified]);
  
  // UPDATED: Handle purchase with KYC check
  const handlePurchase = async () => {
    ...
    // ADDED: Check if KYC is required but user isn't verified
if (!isKYCVerified) {
  setPendingPurchase({ assetId: asset.assetId, shareAmount });
  setShowKYC(true);
  return; 
}
  ...
  // ADDED: Handle successful KYC
  const handleKYCSuccess = async () => {
    setIsKYCVerified(true);
    setShowKYC(false);
    
    // Retry the purchase after KYC is complete
    if (pendingPurchase) {
      try {
        setLoading(true);
        await onPurchase(pendingPurchase.assetId, pendingPurchase.shareAmount);
        showNotification(
          `Successfully acquired ${formatPercentage(percentage)}% ownership of ${asset.assetName}!`, 
          'success'
        );
          ...
          <div className="p-8">
            ...
            {asset.requiresPurchaserKYC && (
              <div className={`flex items-start gap-3 p-4 mb-6 ${
                checkingKYC ? 'bg-neutral-900 border border-neutral-800' :
                isKYCVerified ? 'bg-green-900/10 border border-green-900/30' :
                'bg-amber-900/10 border border-amber-900/30'
              }`}>
                {checkingKYC ? (
                  <>
                    <Loader2 className="w-5 h-5 text-neutral-400 animate-spin flex-shrink-0 mt-0.5" />
                    <div>
                      <p className="text-sm font-light text-neutral-400">Checking verification status...</p>
                    </div>
                  </>
                ) : isKYCVerified ? (
                  <>
                    ...
                    <div>
                      <p className="text-sm font-light text-green-400 mb-1">Identity Verified</p>
                      <p className="text-xs font-light text-neutral-400">
                        You're verified and ready to purchase this asset.
                      </p>
                    </div>
                  </>
                ) : (
                  <>
                    ...
                    <div>
                      ...
                      <p className="text-xs font-light text-neutral-400">
                        This asset requires identity verification before purchase. You'll be prompted to verify when you click purchase.
                      </p>
                    </div>
                  </>
                )}
          ...
          <div className="sticky bottom-0 bg-black border-t border-neutral-900">
            <div className="flex gap-4 p-8">
              ...
              <button
                ...
                disabled={loading || shareAmount < 1 || checkingKYC}
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    <span>Verify & Acquire {formatPercentage(percentage)}% Ownership</span>
                  </>
                ) : (
                  <>
